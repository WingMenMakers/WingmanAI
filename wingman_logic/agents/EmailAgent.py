...
from wingman_logic.tools.mailTool import MailTool # Corrected import path for clarity
...
class EmailAgent:
    ...
    def _handle_reply_email(self, params: Dict) -> str:
        """
        Handles the logic for replying to a specific email.
        Auto-resolves the recipient if not provided in the prompt.
        """
        message_id = params.get("message_id")
        content_prompt = params.get("query") or params.get("body")
        
        # 1. Validate ID
        if not message_id or "REQUIRED" in message_id:
             return "Agent Error: MISSING_DATA_ID; I need the specific email ID to reply to."
        
        # 2. Validate Content
        if not content_prompt:
             return "Agent Error: MISSING_DATA_CONTENT; What should I say in the reply?"

        # 3. Resolve Recipient (The "To" address)
        # The LLM might have put the name in 'params["to"]' or 'params["sender"]'
        recipient = params.get("to") or params.get("sender")
        
        # If the LLM didn't extract a recipient (e.g., user said "Reply to this"), 
        # we must fetch the original email to see who sent it.
        if not recipient or '@' not in recipient:
             try:
                 original_mail = self.mail_tool.get_mail_body_by_id(message_id)
                 if not original_mail or not original_mail.get("sender"):
                     return f"Agent Error: EMAIL_NOT_FOUND; Could not find valid sender for ID {message_id}"
                 
                 # Use the sender of the original mail as the recipient
                 recipient = original_mail.get("sender") 
                 
             except Exception as e:
                 return f"Agent Error: RESOLUTION_FAILURE; Could not fetch original email: {str(e)}"

        # 4. Compose the Reply Body
        # We pass 'None' for subject because MailTool automatically prepends "Re:" to the original subject
        composed = self._generate_email_content_json(to=recipient, context=content_prompt, subject=None)
        
        if not composed.get("success"):
            return f"Agent Error: COMPOSE_FAILURE; {composed.get('error')}"

        # 5. Execute Reply
        try:
             # MailTool.reply_to_email handles the threading headers automatically
             self.mail_tool.reply_to_email(message_id=message_id, to=recipient, body=composed["body"])
             
             return f"RAW_STATUS: REPLY_SENT; Recipient: {recipient}"
             
        except Exception as e:
             return f"Agent Error: REPLY_FAILURE; {str(e)}"
