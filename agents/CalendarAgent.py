...
from Tools.CalendarTool import GoogleCalendarTool 
...
class CalendarAgent:
    ...
    def _analyze_query_to_json(self, user_query: str) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            '   "query_context": "..." (The user\'s original intent/keywords for the event)\n'
            '}\n\n'
    ...
    def handle_query(self, query: str, context: Any = None) -> Dict[str, Any]:
        ...
        if isinstance(resolved_task_or_error, str):
            # This is a failure returned by the resolver (e.g., AMBIGUOUS_MATCH)
            # We must prioritize INCOMPLETE over FATAL_ERROR here.
            ...
            if resolved_task_or_error.startswith("Agent Error: AMBIGUOUS_EVENT_MATCH") or \
               ...
               resolved_task_or_error.startswith("Agent Error: MISSING_DATA"):
                ...
                return {"status": "INCOMPLETE: MISSING_DATA", "action": action, "context": resolved_task_or_error}
            
            # Catch all other Agent Errors from the resolver as FATAL
            return {"status": "FATAL_ERROR: RESOLVER_FAIL", "action": action, "context": resolved_task_or_error}
        ...
        if action == "create" and (start_time == "MISSING_TIME" or not start_time):
             # This means the planner failed and the resolver couldn't fix it.
             return {"status": "INCOMPLETE: MISSING_DATA", "action": action, "context": f"Agent Error: MISSING_DATA_CALENDAR_TIME; Event name: {event_name or 'N/A'}"}
        ...
        try:
            if action == "create":
                ...
                if response and "id" in response:
                    context = f"RAW_STATUS: EVENT_CREATED; ID: {response['id']}; Summary: {event_name}; Start: {start_time}"
                    return {"status": "COMPLETE: RAW_STATUS", "action": action, "context": context}
                else:
                ...
                if update_result and "id" in update_result:
                    context = f"RAW_STATUS: EVENT_UPDATED; ID: {update_result['id']}; New Time: {start_time}"
                    return {"status": "COMPLETE: RAW_STATUS", "action": action, "context": context}
                
                # If tool returns None/False, it's NOT_FOUND (unambiguous, so treat as INCOMPLETE/recoverable by asking user)
                context = f"Agent Error: EVENT_NOT_FOUND; Target: {event_name}; Query: {query}"
                ...
                if delete_result is True:
                    context = f"RAW_STATUS: EVENT_DELETED; Summary: {event_name}"
                    return {"status": "COMPLETE: RAW_STATUS", "action": action, "context": context}
                
                # If tool returns None/False (Not Found)
                context = f"Agent Error: EVENT_NOT_FOUND; Target: {event_name}; Query: {query}"
                return {"status": "INCOMPLETE: MISSING_DATA", "action": action, "context": context}
                ...
                if not events_list:
                    context = f"RAW_DATA: NO_EVENTS_FOUND; Range: {start_time} to {end_time}"
                    return {"status": "COMPLETE: RAW_DATA", "action": action, "context": context}
                
                # Return raw event list (complex data needing Director formatting)
                return {"status": "COMPLETE: RAW_DATA", "action": action, "context": events_list}

            else:
                return {"status": "FATAL_ERROR: UNKNOWN_ACTION", "action": action, "context": f"Action '{action}' not supported."}

        except Exception as e:
            # Catch all Tool exceptions (raised by CalendarTool)
            return {"status": "FATAL_ERROR: TOOL_FAIL", "action": action, "context": f"CalendarTool failed: {str(e)}"}